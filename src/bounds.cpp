...
// empty constructor
CircularBounds::CircularBounds(double _radius) : Bounds(), _radius(_radius) {}

GetSet(CircularBounds, double, radius);

void CircularBounds::scale(double factor) { _radius *= factor; }
Vec2 CircularBounds::size() { return Vec2(_radius, _radius) * 2; }

RectangularBounds::RectangularBounds(Vec2 _size)
    : PolygonBounds(), _size(_size) {}

void RectangularBounds::scale(double factor) { _size = _size * factor; }

GetSet(RectangularBounds, Vec2, size);

std::vector<Vec2> RectangularBounds::vertices() {
    return {_size * -0.5, Vec2::cross(_size) * -0.5, Vec2::cross(_size) * 0.5,
            _size * 0.5};
}
...
ConvexBounds::ConvexBounds(std::vector<Position> _vertices, Vec2 _size)
    : PolygonBounds(), _vertices(_vertices), _size(_size) {}
...
unsigned int ConvexBounds::count() { return _vertices.size(); }
