...
Region::Region(Position position, Bounds *bounds)
    : position(position), _bounds(bounds) {}

Bounds *Region::bounds() { return _bounds; }
...
std::vector<Position> PolygonRegion::vertices() {
    ...
    std::transform(out.begin(), out.end(), out.begin(),
                   std::bind(&PolygonRegion::translate_vertex, this,
                             std::placeholders::_1));
    return out;
}
...
Vec2 PolygonRegion::min_translate(PolygonRegion *solid) {
    ...
    double scalar_proj =
        Vec2::dot(vcenter, min_axis) / Vec2::dot(vcenter, vcenter);
