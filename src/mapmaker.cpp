...
int main(int argc, char *argv[]) {
    ...
    Camera camera =
        Camera(&world, player.occupies.region->position,
               new RectangularBounds({10, 10}), &SCREEN_WIDTH, &SCREEN_HEIGHT);
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e) != 0) {
            if (e.type == SDL_MOUSEBUTTONDOWN) {
                ...
                case SDLK_ESCAPE:
                    ...
                    break;
                case SDLK_SPACE: {
                    std::vector<Position> vbounds(
                        vertices.begin(), vertices.begin() + vertices_size);
                    ...
                    solids->push_back(new PolygonRegion(
                        center, new ConvexBounds(vbounds, {r - l, u - d})));
                    vertices.clear();
                    vertices_size = 0;
                    break;
                }
                case SDLK_LEFT:
                    vertices_size -= 1;
                    vertices_size =
                        Constrain(vertices_size, 0, (int)vertices.size());
                    break;
                case SDLK_RIGHT:
                    vertices_size += 1;
                    vertices_size =
        ...
        // drawing grid
        {
            ...
            int j = 0;
            if (camera.region->position.x -
                    camera.region->bounds()->size().x / 2 >
                0)
                j = std::ceil(camera.region->position.x -
                              camera.region->bounds()->size().x / 2);
            else
                j = 1 + std::floor(camera.region->position.x -
                                   camera.region->bounds()->size().x / 2);
            for (int i = 0; i < camera.region->bounds()->size().x; i++) {
                int a =
                    (int)((SCREEN_WIDTH / camera.region->bounds()->size().x) *
                              (j + i - camera.region->position.x) +
                          SCREEN_WIDTH / 2.0);
                SDL_RenderDrawLine(renderer, a, 0, a, SCREEN_HEIGHT);
            }
            if (camera.region->position.y -
                    camera.region->bounds()->size().y / 2 >
                0)
                j = std::ceil(camera.region->position.y -
                              camera.region->bounds()->size().y / 2);
            else
                j = 1 + std::floor(camera.region->position.y -
                                   camera.region->bounds()->size().y / 2);
            for (int i = 0; i < camera.region->bounds()->size().y; i++) {
                int a =
                    (int)(SCREEN_HEIGHT / 2.0 -
                          (SCREEN_HEIGHT / camera.region->bounds()->size().y) *
                              (j + i - camera.region->position.y));
        ...
        for (unsigned int j = 0; j < solids->size(); j++) {
            ...
            SDL_SetRenderDrawColor(renderer, val, val / 2, (val * 2) % 255,
                                   255);
            ...
            SDL_Rect rect = camera.screen_transform(
                (*map.solids())[j]->position, (*map.solids())[j]->bounds());
